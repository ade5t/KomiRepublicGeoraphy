// JavaScript Document
			...
			{
				document.getElementById("waitmessage").innerHTML = "Идет загрузка...";
			...
			function OKClick()
			{
				...
				{
					document.getElementById("worderror").innerHTML = "Слово, которое вы ввели, содержит недопустимые символы. Пожалуйста, введите только буквы в поле выше.";
					document.getElementById("worderror").style.display = "block";
					return;
				}
				if (TheirWord.length < WordLength[CurrentWord])
				{
					document.getElementById("worderror").innerHTML  = "Вы не написали достаточно букв. Это слово имеет " + WordLength[CurrentWord] + " букв.";
					document.getElementById("worderror").style.display = "block";
					return;
				}
				if (TheirWord.length > WordLength[CurrentWord])
			...
			function CheckClick()
			{
				...
				if (ErrorsFound > 0 && EmptyFound > 0)
					document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибка") + " и " + EmptyFound + (EmptyFound > 1 ? " слов надо " : " слова надо") + " решить.";
				else if (ErrorsFound > 0)
					document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибок") + " были найдены.";
				else if (EmptyFound > 0)
					document.getElementById("welcomemessage").innerHTML = "Ошибок не обнаружено, но " + EmptyFound + (EmptyFound > 1 ? " слов надо" : " надо") + " решить.";
